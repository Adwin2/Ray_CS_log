...
namespace bydb {
	...
	bool Parser::parseStatement(std::string query) {
		...
		if (result_ -> isValid()) {
			...
			std::cout << "[BYDB-Error] Failed to parse sql statement." 
				<<std::endl; 
		}
	...
	bool Parser::checkSelectStmt(const SelectStatement* stmt) {
		...
		if (stmt -> groupBy != nullptr) {
			std::cout << "[BYDB-Error] Do not support 'Group By' clause." 
				<<std::endl;
			return true;
		}
		
		if (stmt -> setOperations != nullptr) {
			std::cout << "[BYDB-Error] Do not support set operations like 'UNION', " "'Intersect', etc ."
				<<std::endl;
			return true;
		}
		
		if (stmt -> withDescriptions != nullptr) {
			std::cout << "[BYDB-Error] Do not support 'with' clause."
				<<std::endl;
			return true;
		}

		if (stmt -> lockings != nullptr) {
			std::cout<< "[BYDB-Error] Do not support 'lock' clause." 
				<< std::endl;
			return true;
		}
	...
	bool Parser::checkInsertStmt(const InsertStatement* stmt) {
		 ...
		 *  if value was not provided for some columns, add NULL expr for them.
		 */
	...
	Table* Parser::getTable(TableRef* table_ref) {
		if(table_ref->type != kTableName) {
			std::cout<<"[BYDB-Error] Only support ordinary table."
				<<std::endl;
			return nullptr;
		}
	...
	bool Parser::checkColumn(Table* table, char* col_name) {
		...
		std::cout<<"[BYDB-Error] Can not find column "
			<<col_name
			<<" in table "
	...
	bool Parser::checkExpr(Table* table, Expr* expr) {
		switch(expr->type) {
			...
			default: 
								 std::cout << "[BYDB-Error] Unsupport operation "
	...
	bool Parser::checkCreateStmt(const CreateStatement* stmt) {
		switch (stmt->type) {
			case kCreateTable:
				...
				break;
			default:
				std::cout<< "[BYDB-Error] Only support 'Create Table'."
					<< std::endl;
				return true;
		}
	...
	bool Parser::checkCreateTableStmt(const CreateStatement* stmt ) {
		if (stmt->schema == nullptr || stmt->tableName == nullptr) {
			std::cout << "[BYDB-Error] Schema and table name should be specified in the query"
				", like 'db.t'."
				<<std::endl;
			return true;
		}
		...
		if (g_meta_data.getTable(stmt ->schema, stmt -> tableName) != nullptr && !stmt->ifNotExists) {
			std::cout <<"[BYDB-Error] Table "
				<<TableNameToString(stmt->schema, stmt->tableName)
		...
		if (stmt-> columns == nullptr || stmt -> columns->size() == 0) {
			std::cout << "[BYDB-Error] Valid column should be specified in 'Create Table' statement."
				<< std::endl;
			return true;
		}
	...
	bool Parser::checkCreateIndexStmt(const CreateStatement* stmt) {
		if (g_meta_data.getIndex(stmt->schema, stmt->tableName, stmt->indexName) != nullptr && !stmt -> ifNotExists ) {
			std::cout <<"[BYDB-Error] Index " 
				<<stmt->indexName
				<< " of "
				<<TableNameToString(stmt->schema, stmt->tableName)
				<< " already existed !"
	...
	bool Parser::checkDropStmt(const DropStatement* stmt) {
		switch (stmt -> type) {
			case kDropTable: {
								 if(g_meta_data.getTable(stmt->schema, stmt->name) == nullptr && !stmt -> ifExists){
									 std::cout<<"[BYDB-Error] Table "
										 <<TableNameToString(stmt->schema, stmt->name) <<" did not exist!"<<std::endl;
									 return true;
								 }
								 break;
							 }
			case kDropSchema: {
								  if (!g_meta_data.findSchema(stmt->schema)&&!stmt->ifExists) {
									  std::cout<<"[BYDB-Error] Schema "<<stmt->schema << "did not exist!"<<std::endl;
									  return true;
								  }
								  break;
							  }
			case kDropIndex: {
								 if(g_meta_data.getIndex(stmt->schema, stmt->name,stmt->indexName) == nullptr && !stmt->ifExists) {
									 std::cout<<"[BYDB-Error] Index "
										 <<stmt -> indexName
										 <<" of "
			...
			default:
							 std::cout <<"[BYDB-Error] Not support drop statement " << DropTypeToString(stmt->type) <<std::endl;
							 return true;
		}
		return false;
	}
